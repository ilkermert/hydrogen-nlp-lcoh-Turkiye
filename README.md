...
1. **Text-Mining SWOT Analysis** — Turkish hydrogen policy documents classified into Strength/Weakness/Opportunity/Threat categories and sub-themes using a hybrid keyword + zero-shot pipeline, validated by expert annotation (Cohen's κ = 0.81).
2. **Stochastic LCOH Modeling** — Monte Carlo simulation (N = 10,000) propagating uncertainty in CAPEX, SEC, electricity price, operating hours, OPEX, discount rate, and plant lifetime for a 20 MW PV-coupled alkaline electrolysis plant (2025–2050), with first-order Sobol sensitivity analysis.
